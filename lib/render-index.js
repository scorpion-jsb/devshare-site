...
function renderIndex (options) {
  ...
  var appData = options.dev
    ? ''
    : `<script>window.__INITIAL_STATE__ = ${JSON.stringify(options.appData)};</script>`
  return indexMarkup
    ...
    .replace('<!-- error tracker -->',
      options.dev
      ? ''
      : '<!-- TrackJS -->\n' +
      `<script>\n` +
        `\t<script type="text/javascript">window._trackJs = { token: ${config.trackJsToken} };</script>\n` +
